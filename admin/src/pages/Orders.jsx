...
const Orders = ({ token }) => {
  ...
  const statusHandler = async (event, orderId) => {
    try {
      const response = await axios.post("http://localhost:5000/api/order/status", { orderId, status: event.target.value }, { headers: { token } })
      if (response.data.success) {
        ...
        {orders.map((order, index) => (
          <div key={index} className='grid grid-cols-1 sm:grid-cols-[0.5fr_2fr_1fr] lg:grid-cols-[0.5fr_2fr_1fr_1fr_1fr] gap-4 items-start border border-gray-100 p-6 rounded-2xl hover:border-black transition-all'>
              ...
              <div className='mb-3'>
                {order.items.map((item, index) => (
                  <p className='text-sm font-semibold text-zinc-800' key={index}>
                    {item.name} x {item.quantity} <span className='text-gray-400 text-xs'>({item.size})</span>
            ...
            <div className='text-sm'>
                ...
                <p className='font-bold text-zinc-600'>{order.paymentMethod}</p>
            </div>
